...
export function summarize(shifts: Shift[]): WorkerSummary[] {
  ...
  for (const s of shifts) {
    ...
    if (!row) {
      row = { userId: s.userId, userName: s.userName, days: 0, workMin: 0, travelMin: 0, lunchMin: 0, shifts: [] }
...
export { formatMinutes, minutesToDecimal, travelMinutes, workedMinutes }
