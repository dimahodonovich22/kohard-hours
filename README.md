...
**Админ (владелец)**: подтверждает регистрации, создаёт работников напрямую, видит кто сейчас на объектах, смотрит отчёты за неделю/месяц с часами, заработком (€), фотографиями и серверными метками времени (антифрод), исправляет ошибки (в т.ч. ставку), выгружает PDF и .xlsx (открывается в Google Таблицах).
